...
# ╔═╡ 6ef0eb34-f794-4b29-9e01-57e10cfc1f9d
...
# ╔═╡ dadf72c0-b61b-4dd1-873d-bfe81deae923
temperature_factor(t) = 3^((t - 6.3) / 10)

# ╔═╡ 7a406bc9-f21f-4865-bbb4-1474ab515050
begin
...
# ╔═╡ 8478cd48-b628-4c8e-b0a2-f684227d9093
Δ = laplacian_matrix(N, dx)

# ╔═╡ c052838a-ef90-46c1-b88b-ff65e264f551
...
# ╔═╡ 885e8795-f310-48dd-91c7-b18ef9886577
...
# ╔═╡ 348a8ce8-660e-4859-8a0f-f5a35f55c8fe
md"Initial conditons"

# ╔═╡ ceea653e-cdc5-4ca4-b862-bd84d972de65
u0_guess = [V_rest, n_inf(V_rest), m_inf(V_rest), h_inf(V_rest)]

# ╔═╡ b84ff8ce-0c66-47f2-8e96-64185226a107
...
# ╔═╡ a6dfb4ca-0065-4efa-a295-7568eac31d89
tspan = (0.0, 15.0) # [ms]
...
# ╔═╡ bb059820-fbf2-491c-9af0-7fe07638d9df
algos = [
	# implicit
	TRBDF2(),         # stiff trapez. BDF
	#ROS3P(),   # Rosenbrock 3, much slower than TRBDF2
	...
	AutoTsit5(TRBDF2()) # auto switching impl/expl
]

# ╔═╡ c24fe8c4-c458-4da8-859a-cdc78d0801a2
md"Toleranzwerte (da nur testweise, abstol=reltol)"

# ╔═╡ 915e4e2f-51ae-42dd-8533-9d1c90afa242
abstols = 10.0 .^ (-3:-2:-10)

# ╔═╡ 87de4fe8-ad79-4d59-a440-465bb350a527
results = DataFrame(
    algo      = String[],
	tol    = Float64[],
	h = Float64[],
...
# ╔═╡ d6224d35-f6f2-4dd8-8f60-f1628d4ca906
stepsize_plot = plot(
...
# ╔═╡ 36460a53-61d0-4625-b458-e553c0e3a23d


# ╔═╡ 4290950d-002b-49e0-b752-274a1589deef
md"Initial zum vorkompilieren"

# ╔═╡ dcdce9b4-1587-4087-9d18-06713ca87ccf
for alg in algos
	@info alg
	solve(probPDE, alg; abstol=10e-4, reltol=10e-3)
end

# ╔═╡ 2cfdacbb-61ed-4cc4-baea-9e685b1395cf
md"Loop über die vreschiedenen Toleranzen und Algorithmen"

# ╔═╡ 81d58c2c-622e-45e5-841b-376554689516
...
for (i, abstol) in enumerate(abstols)
	...
	for alg in algos
	    ...
	    trial = @benchmark solve($probPDE, $alg;
	                             abstol = $abstol,
	                             reltol = $reltol,
	                             save_everystep = true) samples=20
		sol = solve(
			probPDE, 
			alg; 
			abstol = abstol, 
		...
		push!(results, (
		    ...
		    std(trial).time,        # std dev of runtime (ns)
		...
		npzwrite("outputs/stepsize_$name_alg $abstol .npz", Δt)
	...
	plot!(
		...
		size=(800, 700 * length(abstols)),
		subplot = i
	)
end

# ╔═╡ 8059028e-b7e6-49f6-9b07-ed74b7e47a20
@info results

# ╔═╡ 99176db8-2207-4066-80b0-8b8713810853
CSV.write("outputs/hodgkin_huxley_solver_pde_analysis.csv", results)

# ╔═╡ 346e9f9b-82da-4359-871e-ab62f1b60918
md"Könnten die impliziten Verfahren durch bessere Löser/andere Hyperparameter beschleunigt werden ?"

# ╔═╡ 8ea3a25d-43ee-4dfa-b240-ca15e5ce2ea8
begin
	stepsize_plot
	#plot!(stepsize_plot)
	savefig(stepsize_plot, "outputs/stepsize_plot.pdf")
end

# ╔═╡ 291160bd-a86f-4ff2-9cf7-f924dfe677fa
begin
...
p_time = plot(
    ...
    title  = "Median runtime vs tolerance",
	xticks = (10.0 .^ (-10:-4), string.(10.0 .^ (-10:-4)))
)
...
# combine into a 3-row figure

end

# ╔═╡ 850e7aa7-6e7b-4e21-90e5-8f7e76d8aec4
begin
	plot(p_time, p_allocs, p_bytes; layout = (3, 1), legend = :topleft, size=(800,1400),background_color_legend = nothing, markersize=7, alpha=0.82)
	savefig("outputs/algorithm_time_allocs_byte_plot.pdf")
end

# ╔═╡ 1a3cdab9-5689-4fc8-8e5d-61bcb5ad7de9
md"Reference solution"

# ╔═╡ d80d69f5-1830-4555-afad-a3cecb7353e1
sol = solve(probPDE, TRBDF2(), abstol=10e-12, reltol=10e-8)

# ╔═╡ 07433109-7d69-46aa-83d7-5e493cc87b4d
begin
    Nt = length(sol.t)
    Nspace = N
    ...
    PDE_plot = plot(pV, pn, pm, ph, pNa, pK, pL, layout=(3,3), size=(1800, 1800))
	savefig(PDE_plot, "outputs/reference_solution_plot.pdf")
end
...
manifest_format = "2.0"
project_hash = "861b81a82f09fdc92347758c5c770a8746768cc2"
