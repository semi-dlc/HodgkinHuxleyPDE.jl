...
begin
	...
	V_rest = -63.05  # See to which value it converges in steady state
...
Furthermore, a singular spike is also achieved by setting the initial potential to a value above the resting potential.
"

# ╔═╡ 321c84ac-d12a-46e8-bb50-62a654459841
V_rest_values = -70.0:1.001:20.0  # You can choose the resolution
...
# ╔═╡ 739989c1-fac5-4187-a346-bf9c9275d94d
anim = @animate for V_rest in V_rest_values
    # Initial conditions
    u0 = [V_rest, n_inf(V_rest), m_inf(V_rest), h_inf(V_rest)]
    prob = ODEProblem(hh!, u0, (0.0, 200.0), p)
    sol = solve(prob, Rosenbrock23())

    # Extract time series
    t = sol.t
    V = [sol[i][1] for i in eachindex(sol)]
    n = [sol[i][2] for i in eachindex(sol)]
    m = [sol[i][3] for i in eachindex(sol)]
    h = [sol[i][4] for i in eachindex(sol)]

    # Voltage plot
    pv = plot(t, V,
        xlabel = "Time (ms)", ylabel = "V (mV)",
        label = "V_rest = $(round(V_rest, digits=1)) mV",
        ylim = (-80, 60),
        title = "Membrane Voltage")

    # Gating variables plot
    pnmh = plot(t, [n, m, h],
        xlabel = "Time (ms)", ylabel = "Gating vars",
        label = ["n" "m" "h"],
        ylim = (0, 1),
        title = "Gating Variables")

    plot(pv, pnmh, layout = (1, 2), size = (900, 400))
end

# ╔═╡ cf179403-e0be-4740-bbb9-db230e7c9619
anim

# ╔═╡ ddd95027-44fc-46df-ab0a-ecaf038185d3
gif(anim, "V_rest_all_variables.gif", fps = 5)
...
manifest_format = "2.0"
project_hash = "2b4972f17942b74bbb74f0f48072f3bb7f5e8a52"
