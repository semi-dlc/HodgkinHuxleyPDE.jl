...
# ╔═╡ b6d32bdb-0f3b-4c24-9d80-e07080cec819
begin
...
# ╔═╡ 278eefa4-a31c-45a8-8242-7e4ef91d9f43
const GIANT_AXON = true

# ╔═╡ a4d1ece9-38f5-40b0-827d-014d77031f28
if GIANT_AXON
...
# ╔═╡ 7e3479c0-206e-4a21-89eb-70e1753eadff
begin
...
# ╔═╡ ddeabbe5-1c7d-4a03-9642-bd02997779eb
...
# ╔═╡ f49fcec7-c108-43d3-9335-83f8fd974ab4
# Main HH ODE system 
...
# ╔═╡ 33480651-43d8-450b-abf6-f3246ae03c2c
...
# ╔═╡ f86093eb-b3a1-4fc3-98a8-c4c2ed32aaad
#@bind T Slider(0.0:0.1:30.0; default=6.3, show_value=true)
T=12.5

# ╔═╡ 72da6675-7e9f-4849-8031-c9c1bf3c1400
md"Problem: h<0"

# ╔═╡ b2f0e1ab-abbe-4604-9f34-cfe25b2ec0c3
@bind I_stim Slider(-10:0.1:20; default=0., show_value=true)

# ╔═╡ 23ec6fde-0a6a-46d2-83ba-c3160480c0ff
I_base = -20.

# ╔═╡ 04d1396d-bfc8-4236-a183-c123779cf571
begin
	# Setup parameters
...
# ╔═╡ 6ea1ba41-6896-4eb2-9abe-6e4afeacc15e
begin
	# ODE Problem and solution (no stimulation)
tspan = (0.0, 100.0)
probODE = ODEProblem(hh!, u0, tspan, p)
sol = solve(probODE, TRBDF2(), dtmax=0.1)
end

# ╔═╡ c570e0fb-0472-4059-b5b6-8be1e917db30
plot(sol.t, getindex.(sol.u, 1), xlabel="Time (ms)", ylabel="Membrane Potential (mV)",
    label="V(t)", title="Giant Axon at T=$T °C, I_stim=$I_stim μA/cm²", size=(800, 800))

# ╔═╡ 2c08b79d-09cd-4e12-84fc-159dd205c6f7
plot(sol.t, [getindex.(sol.u, 2),getindex.(sol.u, 3),getindex.(sol.u, 4)], xlabel="Time (ms)", ylabel="Gating Variables",
    label=["n" "m" "h"], title="Giant Axon at T=$T °C, I_stim=$I_stim μA/cm²", size=(800, 800))

# ╔═╡ 5e4903c8-86f5-41fa-a192-4951773dd5de
plot(sol, vars=(2, 3, 4), 
    xlabel="n", ylabel="m", zlabel="h", title="Giant Axon at T=$T °C, I_stim=$I_stim μA/cm²", size=(800, 600))

# ╔═╡ 8a79a056-0089-4158-8d1b-76b7efd60469
begin
	t_vals = sol.t
...
# ╔═╡ e29fe112-d2e8-4fda-b6ac-def2c7514a01
begin
...
manifest_format = "2.0"
project_hash = "c51d1c0581a22f856565ab0adfa58124cdf48062"
